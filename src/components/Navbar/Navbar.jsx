...
const Navbar = () => {
    ...
    // Function to check if the current path is active for 'Berita & Pengumuman' or 'Pengumuman'
    const isActiveRoute = (route) => {
        // Khusus untuk Berita & Pengumuman
        if (route === '/berita' || route === '/berita/pengumuman') {
            return pathName.startsWith('/berita');
        } else if (route === '/dokumen') {
    ...
    return (
        <>
            ...
            <div className="sticky top-3 z-50 hidden lg:block">
                <div
                    className={`mx-auto w-[1200px] rounded-2xl transition-all ${isScrolled ? "bg-white/10 shadow-md backdrop-blur-lg" : ""
                        }`}
                >
                                ...
                                <ul className="menu menu-horizontal px-1">
                                    {NavItem.slice(0, 4).map((item, i) => (
                                        <li key={i} className="relative">
                                            <Link href={item.url}>
                                                <span
                                                    className={`relative transition-colors duration-300 ease-in-out hover:text-textAccent ${isActiveRoute(item.url)  // Memeriksa apakah URL aktif sesuai dengan kondisi khusus
                                                ...
                                                >
                                                    ...
                                                    {/* Border Bottom */}
                                                    <span
                                                        className={`absolute -bottom-2 left-0 h-[2px] w-full bg-gray-500 transition-all duration-300 ease-in-out dark:bg-textAccent ${isActiveRoute(item.url)
                                                                ? "scale-x-100 "
                                                                : "scale-x-0"
                                                            }`}
                                                        style={{
                                                            transformOrigin: isActiveRoute(item.url)
                                                                ? "left"
                                                                : "right",
                                                        }}
                                                    />
